...
model LargeAmplitudePendulum "Pendulum with pi/2 Amplitude and Period from Elliptic Integral"
  ...
  record BodyParams
    ...
    parameter Real I=I_cm+m*L*L "kg m^2, total I (don't change)";
    parameter Real T0=2.0*Modelica.Constants.pi*sqrt(I/(m*L*Modelica.Constants.g_n)) "s, small-angle period (don't change)";
  end BodyParams;
...
equation
  ...
  Documentation(info = "<html><head></head><body>This example involves a pendulum with pi/2 (=theta<sub>0</sub>) amplitude. A mass (m,I_cm) is suspended a distance L from a pivot. From the conservation of total (kinetic + potential) energy, one finds:<div><br></div><div>dtheta/dt = sqrt[(2mgL/I)(cos(theta) - cos(theta<sub>0</sub>))]&nbsp;</div><div><br></div><div>After differentiating and doing some algebra, one arrives at the following DE:<br><div><br></div><div>d<sup>2</sup>theta/dt<sup>2</sup> + (mgL/I) sin(theta) = 0&nbsp;</div><div><br></div><div>The period can be found by integrating the dt/dtheta expression above, performing a change of integration variable, and using a trig half-angle formula. The result is a complete elliptic integral, K(k):</div><div><br></div><div>T = 4 sqrt(I/mgL) K(sin(theta<sub>0</sub>/2))&nbsp;</div><div><br></div><div>Plotting the period, T, as a function of starting angle (assuming dtheta/dt=0 at theta<sub>0</sub>):</div><div><img src=\"modelica://GNU_ScientificLibrary/Examples/specfunc/period_vs_theta0.png\"></div><div><br></div><div>Near theta<sub>0</sub>=0, one finds the solution (T<sub>0</sub>~2s) for the small-angle approximation: sin(theta) ~ theta.</div></div><div><br></div><div>The period of the swinging-body simulation is found to match the calculation of the period from the max angle:</div><div><img src=\"modelica://GNU_ScientificLibrary/Examples/specfunc/thetaT_vs_t.png\"></div></body></html>"));
